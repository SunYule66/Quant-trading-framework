...
def get_coin_name_list(config, online):
    ...
    if not online:
        ...
        volume_forward = 0
    trade_period = input_config.get("trade_period",
                                    input_config.get("global_period"))
    if trade_period is None:
        raise KeyError("`trade_period` or `global_period` must be configured.")
    end = end - (end % trade_period)
    ...
    coins = HistoryManager(input_config["coin_number"], end,
                           ...
                           online=online,
                           market=input_config.get("market", "poloniex")).\
