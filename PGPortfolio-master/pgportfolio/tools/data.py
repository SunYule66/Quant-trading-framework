...
def panel_fillna(panel, type="bfill"):
    ...
    for item in panel.items:
        if type == "both":
            # prefer bfill/ffill methods directly to avoid using deprecated fillna(method=...)
            frames[item] = panel.loc[item].bfill(axis=1).ffill(axis=1)
        else:
            # when `type` is 'bfill' or 'ffill', call the corresponding method
            if type == "bfill":
                frames[item] = panel.loc[item].bfill(axis=1)
            elif type == "ffill":
                frames[item] = panel.loc[item].ffill(axis=1)
            else:
                # fallback to generic fillna behavior for other values
