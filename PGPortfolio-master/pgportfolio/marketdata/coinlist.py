...
class CoinList(object):
    ...
    def _build_exchange_client(self):
        if self._market in ("poloniex", "binance"):
            return Poloniex()
        if self._market in ("okx", "okex"):
            return Okx()
        raise ValueError("Unsupported market '{}'. Allowed markets: poloniex, okx".format(self._market))
