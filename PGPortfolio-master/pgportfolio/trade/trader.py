...
class Trader:
    def __init__(self, waiting_period, config, total_steps=None, net_dir=None,
                 agent=None, initial_BTC=1.0, agent_type="nn", device="cpu"):
        """
        ...
        if agent_type == "traditional":
            ...
            self._rolling_trainer = RollingTrainer(config, net_dir, agent=agent, device=device)
    ...
    def __trade_body(self):
        ...
        remaining = self._period - trading_time
        if remaining > 0:
            logging.info("sleep for %s seconds" % remaining)
        self._steps += 1
        return remaining
    ...
    def start_trading(self):
        try:
            def _should_continue():
                return self._total_steps is None or self._steps < self._total_steps
