...
class BackTest(trader.Trader):
    def __init__(self, config, net_dir=None, agent=None, agent_type="nn"):
        ...
        self._allow_backtest_rolling = bool(
            config["trading"].get("allow_backtest_rolling", False)
        )
    ...
    def rolling_train(self, online_sample=None):
        """
        Optional rolling train during backtest.
        Default remains disabled to avoid test-set leakage unless explicitly enabled in config.
        """
        if not self._allow_backtest_rolling:
            logging.debug("Backtest rolling training disabled to avoid data leakage")
            return
        logging.debug(
            "Backtest rolling training enabled for %s steps",
            self._rolling_trainer.rolling_training_steps,
        )
        self._rolling_trainer.rolling_train(online_sample)
