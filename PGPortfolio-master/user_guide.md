...
* `"input "`
    ...
    * `"market"`
        * data source / exchange adapter to use.
        * supported values: `poloniex` (legacy Binanace proxy) or `okx`.
    * `"online"`
...
3. type `python main.py --mode=train --processes=1`
    ...
    * add "--device=gpu" if your PyTorch environment supports GPU acceleration.
...
## OKX Dataset Pipeline
* Type `python main.py --mode=okx_dataset --dataset_type=historical --algo=1`
  * Downloads the whole range defined in your config straight into the sqlite DB used by the trainer/backtester.
  * Use `--database_file=MyData.db` to write into `database/MyData.db`; omit it to reuse `Data.db`.
* Type `python main.py --mode=okx_dataset --dataset_type=realtime --algo=1 --coins=BTC,ETH --live_steps=24`
  * Starts the OKX realtime collector, writes aligned CSV snapshots under `database/realtime`, and immediately ingests them into sqlite via `CSVDatabaseBuilder`.
  * Override the snapshot folder with `--output_dir` and the CSV import folder with `--source_dir` (defaults to the same path).
* Use `--dataset_type=all` to run both phases back-to-back (historical refresh followed by realtime capture/import).

## Live Trading (OKX)
* Type `python main.py --mode=okx_trade --algo=1`
  * Requires a trained model inside `train_package/1/netfile` and `trading.provider="okx"`.
  * Supply API keys via `pgportfolio/net_config.json` or environment variables `OKX_API_KEY`, `OKX_API_SECRET`, `OKX_PASSPHRASE`.
  * Leave `trading.paper_trading=true` to simulate orders against live prices; set it to `false` **only after** validating with paper trades.
  * Use `--live_steps=n` to limit the number of live bars processed in a session.
