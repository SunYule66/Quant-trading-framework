...
## OKX Data Pipeline
`okx_dataset` bundles historical downloads, realtime snapshots, and CSV-to-sqlite
conversion into a single command so you can quickly refresh the `database/Data.db`
that powers training/backtests.

```
# Historical range defined in your config
python main.py --mode=okx_dataset --dataset_type=historical --algo=1 --database_file=okx.db

# Realtime capture + sqlite import
python main.py --mode=okx_dataset --dataset_type=realtime --algo=1 \
    --coins=BTC,ETH --live_steps=24 --output_dir=database/realtime_okx

# Run both phases sequentially
python main.py --mode=okx_dataset --dataset_type=all --algo=1 --database_file=okx.db
```

- `--dataset_type` accepts `historical`, `realtime`, or `all` to decide which phases run.
- `--database_file` overrides `input.database_file`; omit it to reuse the config default (`database/Data.db`).
- `--coins`, `--live_steps`, and `--output_dir` control the realtime collector; snapshots are ingested automatically with `CSVDatabaseBuilder`.

## Live Trading on OKX
`OkxTrader` remains accessible through the CLI so you can rebalance a trained model
on your OKX account. The trader pulls the same OHLCV windows the dataset pipeline
creates, feeds them through the neural network, and submits market orders via the
official REST API.

```
python main.py --mode=okx_trade --algo=1 --live_steps=100
```

1. Set your API credentials either directly in `pgportfolio/net_config.json` (`trading.api_key`
   etc.) or as environment variables (`OKX_API_KEY`, `OKX_API_SECRET`, `OKX_PASSPHRASE`).
2. Set `trading.provider` to `okx` and disable `paper_trading` to send real orders.
3. Optionally use the OKX demo trading environment via `trading.use_simulated=true`.

⚠️ Always dry-run with `paper_trading=true` before connecting to real capital and add
position/size limits that match your own risk tolerance.
